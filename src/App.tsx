...
import { Phone, Instagram, ArrowUpRight, Clock } from 'lucide-react';
    ...
    <div className="min-h-screen font-sans bg-balance-bg text-balance-dark relative overflow-hidden">
      ...
      <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 px-4 z-10 w-[90%] max-w-7xl mx-auto min-h-screen flex items-center">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20 w-full">
          ...
          >
            ...
            <RunningSinceTimer startDate="2025-09-01T00:00:00" />
          </motion.div>
      ...
      <section id="order" className="py-32 px-4 bg-balance-dark text-balance-bg relative z-10 border-t border-balance-dark">
        <div className="max-w-[90%] 2xl:max-w-7xl mx-auto text-center">
          <span className="text-[10px] uppercase font-bold tracking-widest text-balance-gold mb-6 block">Ready Since September 2025</span>
          <h2 className="font-display italic font-medium text-5xl lg:text-7xl mb-8">
            Amankan <span className="text-balance-gold">Batch Berikutnya.</span>
          </h2>
          <p className="text-xl mb-12 font-light max-w-2xl mx-auto opacity-80">
            Hubungi admin kami untuk cek stok terbaru dan ikut kloter produksi selanjutnya.
          </p>
...
function RunningSinceTimer({ startDate }: { startDate: string }) {
  const calculateElapsedTime = () => {
    const start = new Date(startDate).getTime();
    const now = new Date().getTime();
    const difference = Math.max(now - start, 0);
  ...
  return (
    <div className="border border-balance-dark p-6 mt-8 max-w-sm bg-white/70 backdrop-blur-sm">
      <div className="flex items-center gap-2 mb-6 text-balance-gold border-b border-balance-dark/10 pb-4">
        <span className="text-[10px] font-bold uppercase tracking-widest flex items-center gap-2">
          <Clock className="w-3 h-3" /> Sudah berjalan sejak September 2025
        </span>
      </div>
      <div className="flex gap-4">
        {[
          ...
          { label: 'Menit', value: elapsedTime.mins },
          { label: 'Detik', value: elapsedTime.secs },
        ].map((unit, i) => (
